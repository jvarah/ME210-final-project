...
*/
#define LEFT_INVERT_MASK 0b1000
#define RIGHT_INVERT_MASK 0b0100
#define LEFT_DIR_MASK 0b0010
#define RIGHT_DIR_MASK 0b0001
...
// // Gyro variables
// Example connects this to SCL (A5), and SDL (A4)
// static double gyro_offset; // Used to calibrate gyro position after power on

Drive::Drive(uint8_t left_dir_port, uint8_t left_en_port,
             uint8_t right_dir_port, uint8_t right_en_port) {
  // Gyro initialization
  gyro = Adafruit_BNO055();  // TODO: Not sure that this is the correct
                             // initialization, may need to put the I2C address
                             // in here
  if (!gyro.begin()) {
    Serial.println("No gyro detected");
  }
  gyro.setExtCrystalUse(USE_EXT_CRYSTAL);
  gyro_offset = 0;
...
void Drive::setLeftPower(int8_t power) {
  ...
  analogWrite(l_en_port, abs(power) << 2);
}
...
void Drive::setRightPower(int8_t power) {
  ...
  // Input is between -127 to 128, which shows direction and magnitude
  // Want the power to be between 0 to 255, so multiply abs(input) by 2
  analogWrite(r_en_port, abs(power) << 2);
}
...
double Drive::getAngle() {
  ...
  return euler.z() - gyro_offset; // Assuming this is yaw, needs testing
}

void Drive::setGyroOffset(double offset) {
  gyro_offset = offset; // Can be used so 0 is always when the robot IR sensor is facing the beacon
}
